...
import { chartDataUrl, downloadDataUrl, shareChart } from '@/utils/exportUtils';
import clsx from 'clsx';

const chartIcons: Record<ChartType, typeof BarChart3> = { bar: BarChart3, 'horizontal-bar': BarChart3, line: LineChart, area: LineChart, pie: PieChart, doughnut: PieChart, scatter: ScatterChart, histogram: LayoutGrid, 'box-plot': LayoutGrid, radar: Radar, 'polar-area': Radar, bubble: ScatterChart, funnel: Filter, gauge: Gauge, waterfall: BarChart3, combo: GitBranch, 'stacked-bar': BarChart3, 'stacked-area': LineChart };
...
export const NewChart = () => {
  ...
  const [generated, setGenerated] = useState(Boolean(chartId));
  ...
  useEffect(() => {
    if (projectId && generated && chart && useProjectStore.getState().projects.some((item) => item.id === projectId && item.charts.some((savedChart) => savedChart.id === chart.id))) updateChart(projectId, { ...chart, dataset: table });
  }, [chart, generated, projectId, table, updateChart]);
  ...
  useEffect(() => {
    const handleShortcut = (event: KeyboardEvent) => {
      ...
      if (key === 's') { event.preventDefault(); setSaved(true); window.setTimeout(() => setSaved(false), 2200); }
      if (key === 'e') { event.preventDefault(); setExportOpen(true); }
  ...
  const tableFingerprint = useMemo(() => JSON.stringify({ columns: table.columns, rows: table.rows }), [table]);
  const savedFingerprint = useMemo(() => JSON.stringify({ columns: chart?.dataset?.columns ?? [], rows: chart?.dataset?.rows ?? [] }), [chart?.dataset]);

  if (!chart) return null;
  const option = buildChartOption(table, chart);
  const dataChanged = savedFingerprint !== tableFingerprint;
  const canGenerate = table.rows.length > 0 && table.columns.length > 0 && chart.mapping.valueColumnIds.length > 0 && chart.mapping.valueColumnIds.some((id) => { const index = table.columns.findIndex((column) => column.id === id); return index >= 0 && table.rows.some((row) => Number(String(row[index] ?? '').replace(/[$,%\s,]/g, '')) || row[index] === '0'); });
  const generateChart = () => { if (!canGenerate) { setGenerationStatus('Add at least one row and map a numeric value column first.'); return; } setGenerationStatus('Generating...'); window.setTimeout(() => { updateCurrentChart({ dataset: table }); setGenerated(true); setGenerationStatus('Chart generated.'); }, 120); };
  ...
  const quickExport = () => { try { downloadDataUrl(chartDataUrl(table, chart, { format: 'png', width: 1200, height: chart.customization.height, pixelRatio: 2, background: chart.customization.background, transparent: false, quality: 1 }), `${chart.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'chartgen-chart'}.png`); setExportStatus('Quick PNG export complete.'); } catch { setExportStatus('Quick export failed. Open Advanced export for more options.'); } };
  const share = async () => { try { setExportStatus(await shareChart(chart, project?.name)); } catch (error) { setExportStatus(error instanceof Error ? error.message : 'Sharing failed.'); } };
  const customization = generated ? <CustomizationSidebar chart={chart} columns={table.columns} updateCustomization={updateCustomization} updateSeries={updateSeries} applyPreset={applyPreset} resetCustomization={resetCustomization} savePreset={savePreset} /> : <p className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">Generate a chart to unlock customization.</p>;
  ...
  return <div className={clsx('flex min-h-[calc(100vh-10rem)] flex-col gap-4', fullscreen && 'bg-background p-6')}>
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-accent">Chart studio</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Build a chart from your data</h1>{generationStatus && <p className="mt-1 text-xs text-muted-foreground" role="status">{generationStatus}</p>}{exportStatus && <p className="mt-1 text-xs text-muted-foreground" role="status">{exportStatus}</p>}</div><div className="flex flex-wrap gap-2"><Button variant="ghost" size="sm" disabled={!past.length} onClick={undo} title="Undo"><Undo2 className="h-4 w-4" /></Button><Button variant="ghost" size="sm" disabled={!future.length} onClick={redo} title="Redo"><Redo2 className="h-4 w-4" /></Button><Button variant="outline" size="sm" onClick={duplicate}><Copy className="mr-2 h-4 w-4" />Duplicate</Button><Button variant="outline" size="sm" onClick={enterFullscreen}><Expand className="mr-2 h-4 w-4" />{fullscreen ? 'Exit full screen' : 'Full screen'}</Button><Button variant="primary" size="sm" disabled={!canGenerate || generationStatus === 'Generating...'} onClick={generateChart}>{generationStatus === 'Generating...' ? 'Generating...' : dataChanged ? 'Generate Chart' : 'Generate Chart'}</Button><Button variant="outline" size="sm" disabled={!generated} onClick={quickExport}>Quick PNG</Button><Button variant="outline" size="sm" disabled={!generated} onClick={() => setExportOpen(true)}>Export</Button><Button variant="outline" size="sm" className="xl:hidden" onClick={() => setMobileCustomize(true)}><SlidersHorizontal className="mr-2 h-4 w-4" />Customize</Button><Button size="sm" onClick={() => { setSaved(true); window.setTimeout(() => setSaved(false), 2200); }}><Save className="mr-2 h-4 w-4" />{saved ? 'Saved' : 'Save'}</Button></div></div>
    ...
    <ExportDialog chart={chart} table={table} projectName={project?.name} open={exportOpen} onClose={() => setExportOpen(false)} onShare={share} />
  </div>;
};
...
const EmptyChart = ({ needsGeneration, onUseSample }: { needsGeneration: boolean; onUseSample: () => void }) => <div className="flex flex-1 flex-col items-center justify-center text-center"><div className="rounded-full bg-accent/10 p-4"><BarChart3 className="h-8 w-8 text-accent" /></div><h2 className="mt-4 text-lg font-semibold">{needsGeneration ? 'Your data is ready to generate.' : 'Add data to generate your chart.'}</h2><p className="mt-2 max-w-sm text-muted-foreground">{needsGeneration ? 'Use Generate Chart above to validate the mapping and create the preview.' : 'Connect a dataset, import a file, or use a sample to start building your visualization.'}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Link to="/data"><Button><Table2 className="mr-2 h-4 w-4" />Enter data</Button></Link><Button variant="outline" onClick={onUseSample}><Save className="mr-2 h-4 w-4" />Use sample data</Button></div></div>;
