...
import { BarChart3, ChevronDown, Copy, Expand, Filter, Gauge, GitBranch, LayoutGrid, LineChart, PieChart, Radar, Redo2, RotateCcw, Save, Search, ScatterChart, Table2, Undo2 } from 'lucide-react';
...
import { buildChartOption, chartTypeGroups, createDefaultChart } from '@/utils/chartOptions';
import clsx from 'clsx';

const chartIcons: Record<ChartType, typeof BarChart3> = {
  bar: BarChart3, 'horizontal-bar': BarChart3, line: LineChart, area: LineChart, pie: PieChart, doughnut: PieChart,
  scatter: ScatterChart, histogram: LayoutGrid, 'box-plot': LayoutGrid, radar: Radar, 'polar-area': Radar,
  bubble: ScatterChart, funnel: Filter, gauge: Gauge, combo: GitBranch, 'stacked-bar': BarChart3, 'stacked-area': LineChart,
};
...
export const NewChart = () => {
  ...
  const { currentChart, past, future, setCurrentChart, setChartType, updateCurrentChart, updateMapping, duplicate, reset, undo, redo } = useChartStore();
...
const EmptyChart = ({ onUseSample }: { onUseSample: () => void }) => <div className="flex flex-1 flex-col items-center justify-center text-center"><div className="rounded-full bg-accent/10 p-4"><BarChart3 className="h-8 w-8 text-accent" /></div><h2 className="mt-4 text-lg font-semibold">Add data to generate your chart.</h2><p className="mt-2 max-w-sm text-sm text-muted-foreground">Connect a dataset, import a file, or use a sample to start building your visualization.</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Link to="/data"><Button><Table2 className="mr-2 h-4 w-4" />Enter data</Button></Link><Link to="/data"><Button variant="outline"><UploadIcon />Import CSV</Button></Link><Button variant="outline" onClick={onUseSample}><Save className="mr-2 h-4 w-4" />Use sample data</Button></div></div>;
const UploadIcon = () => <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M12 16V4m0 0L8 8m4-4 4 4M5 20h14" /></svg>;
