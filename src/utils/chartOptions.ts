...
import { colorAt, colorsFor } from './chartColors';
...
export const chartTypeGroups: { name: string; types: { id: ChartType; label: string; description: string }[] }[] = [
  ...
  { name: 'Comparison', types: [{ id: 'stacked-bar', label: 'Stacked bar', description: 'Compare parts to whole' }, { id: 'stacked-area', label: 'Stacked area', description: 'Show cumulative trends' }, { id: 'waterfall', label: 'Waterfall', description: 'Show changes step by step' }, { id: 'combo', label: 'Combo', description: 'Mix bars and lines' }] },
...
const buildBaseChartOption = (table: DataTable, config: ChartConfig): EChartsOption => {
  ...
  const common = { animationDuration: 500, color: colorsFor(selectedSeries.length, config.customization), tooltip };
  if (!selectedSeries.length) return emptyOption('Map a numeric column to preview this chart.');

  switch (config.type) {
    case 'pie': case 'doughnut': return { ...common, title: { text: config.title, left: 'center' }, tooltip: { trigger: 'item' }, series: [{ type: 'pie', radius: config.type === 'doughnut' ? ['42%', '72%'] : '68%', data: labels.map((name, index) => ({ name, value: first.data[index], itemStyle: { color: colorAt(index, config.customization) } })), label: { formatter: '{b}: {d}%' } }] };
    case 'funnel': return { ...common, title: { text: config.title, left: 'center' }, tooltip: { trigger: 'item' }, series: [{ type: 'funnel', left: '10%', width: '80%', minSize: '10%', maxSize: '100%', sort: 'descending', data: labels.map((name, index) => ({ name, value: first.data[index], itemStyle: { color: colorAt(index, config.customization) } })) }] };
    ...
    case 'polar-area': return { ...common, title: { text: config.title, left: 'center' }, angleAxis: { type: 'category', data: labels }, radiusAxis: {}, polar: {}, tooltip, series: [{ type: 'bar', coordinateSystem: 'polar', data: first.data.map((value, index) => ({ value, itemStyle: { color: colorAt(index, config.customization) } })), name: first.name }] };
    ...
    case 'waterfall': {
      ...
      first.data.forEach((value) => { const start = value >= 0 ? running : running + value; starts.push(start); increases.push(value >= 0 ? value : 0); decreases.push(value < 0 ? Math.abs(value) : 0); running += value; });
      return { ...common, title: { text: config.title, left: 'center' }, tooltip, xAxis: { ...axis('Category'), data: labels }, yAxis: axis('Value', 'value'), series: [{ type: 'bar', stack: 'waterfall', itemStyle: { color: 'transparent' }, emphasis: { itemStyle: { color: 'transparent' } }, data: starts }, { type: 'bar', stack: 'waterfall', name: 'Increase', data: increases, itemStyle: { color: colorAt(1, config.customization) } }, { type: 'bar', stack: 'waterfall', name: 'Decrease', data: decreases, itemStyle: { color: colorAt(4, config.customization) } }] };
    }
...
export const buildChartOption = (table: DataTable, config: ChartConfig): EChartsOption => {
  ...
  const series = Array.isArray(base.series) ? base.series.map((item, index) => { const itemObject = typeof item === 'object' ? item as Record<string, unknown> : {}; const id = config.mapping.valueColumnIds[index]; const style = customization.series[id] ?? defaultSeries(colorAt(index, customization)); const data = Array.isArray(itemObject.data) && ['bar', 'horizontal-bar', 'polar-area', 'histogram'].includes(config.type) ? (itemObject.data as unknown[]).map((point, pointIndex) => typeof point === 'object' ? { ...(point as object), itemStyle: { ...((point as { itemStyle?: object }).itemStyle ?? {}), color: colorAt(pointIndex, customization) } } : { value: point, itemStyle: { color: colorAt(pointIndex, customization) } }) : itemObject.data; return { ...itemObject, data, name: style.name ?? itemObject.name, silent: !style.visible, stack: customization.specific.stacked ? 'total' : itemObject.stack, smooth: customization.specific.smooth || itemObject.smooth, showSymbol: customization.specific.symbols, itemStyle: { ...(itemObject.itemStyle as object ?? {}), color: style.color, opacity: style.opacity, borderRadius: config.type.includes('bar') ? customization.specific.borderRadius : undefined }, lineStyle: { ...(itemObject.lineStyle as object ?? {}), width: style.lineWidth, type: style.lineType, color: style.color, opacity: style.opacity }, symbol: style.symbol === 'none' ? 'none' : style.symbol, symbolSize: style.symbolSize, barWidth: `${style.barWidth}%`, areaStyle: style.areaFill || customization.specific.areaFill ? { opacity: style.opacity * 0.3, color: style.color } : itemObject.areaStyle, radius: config.type === 'doughnut' ? [`${customization.specific.pieInnerRadius}%`, `${customization.specific.pieOuterRadius}%`] : config.type === 'pie' ? `${customization.specific.pieOuterRadius}%` : itemObject.radius, startAngle: config.type === 'gauge' ? customization.specific.gaugeStartAngle : config.type === 'pie' || config.type === 'doughnut' ? customization.specific.pieStartAngle : itemObject.startAngle, min: config.type === 'gauge' ? customization.specific.gaugeMin : itemObject.min, max: config.type === 'gauge' ? customization.specific.gaugeMax : itemObject.max, endAngle: config.type === 'gauge' ? customization.specific.gaugeEndAngle : itemObject.endAngle, label: { ...(itemObject.label as object ?? {}), show: customization.dataLabels.show || style.labelVisible, position: config.type === 'pie' || config.type === 'doughnut' ? customization.specific.pieLabelPosition : customization.dataLabels.position, fontSize: customization.typography.dataLabelSize, color: customization.textColor, formatter: customization.dataLabels.numberFormat === 'percentage' ? '{c}%' : undefined } }; }) : base.series;
  return { ...base, backgroundColor: customization.background, textStyle: { color: customization.textColor, fontFamily: customization.typography.fontFamily }, title: { ...(base.title as object ?? {}), text: config.title, subtext: customization.subtitle, left: 'center', textStyle: { color: customization.textColor, fontSize: customization.typography.titleSize, fontFamily: customization.typography.fontFamily }, subtextStyle: { color: customization.textColor, fontSize: customization.typography.subtitleSize } }, grid: { left: customization.margins.left, right: customization.margins.right, top: customization.margins.top, bottom: customization.margins.bottom, show: customization.grid.show, containLabel: true }, xAxis: Array.isArray(xAxis) ? xAxis.map((value) => axisValue(value, customization.xAxis, true)) : axisValue(xAxis, customization.xAxis, true), yAxis: Array.isArray(yAxis) ? yAxis.map((value) => axisValue(value, customization.yAxis, false)) : axisValue(yAxis, customization.yAxis, false), radar: base.radar ? { ...base.radar, radius: `${customization.specific.radarRadius}%` } : base.radar, legend, series } as EChartsOption;
};